            ...
            <div className="flex-1">
              <div className="flex items-start justify-between gap-4">
                ...
                <span className={`px-2 py-1 text-xs rounded-full ${
                  trainer.isAvailable ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                }`}>
              ...
              <div className="mt-3 flex items-center">
                <div className="flex items-center">
                  {[...Array(5)].map((_, i) => (
                    <Star
                      key={i}
                      className={`w-5 h-5 ${i < Math.floor(trainer.rating) ? 'text-yellow-400 fill-current' : 'text-gray-300'}`}
                    />
                  ))}
                </div>
              ...
              <div className="mt-6 flex items-center justify-between">
                <div className="flex items-center">
                  ...
                  <span className="text-xl font-semibold text-gray-900">${trainer.hourlyRate}</span>
                  <span className="text-sm text-gray-600 ml-1">/hour</span>
                </div>
                <Link href="/booking" className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition-colors text-sm font-medium">
                  Book Session
                </Link>
              </div>
